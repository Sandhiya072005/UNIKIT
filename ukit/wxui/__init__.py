...
def maybe_install_desktop(args, parent):
    ...
    with resources.as_file(
            resources.files('ukit.share').joinpath('chirp.desktop')
    ) as desktop_src:
        with open(desktop_src) as f:
            desktop_content = f.readlines()
    with resources.as_file(
            resources.files('ukit.share').joinpath('chirp.ico')) as p:
        icon_path = str(p)
...
def chirpmain():
    ...
    localedir = str(os.path.join(resources.files('ukit'), 'locale'))
    app._lc = wx.Locale()
