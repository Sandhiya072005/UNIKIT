...
from ukit import platform as chirp_platform
from ukit.sources import base
from ukit.wxui import config
from ukit.wxui import bankedit
from ukit.wxui import bugreport
from ukit.wxui import common
from ukit.wxui import clone
from ukit.wxui import developer
from ukit.wxui import memedit
from ukit.wxui import printing
from ukit.wxui import query_sources
from ukit.wxui import radioinfo
from ukit.wxui import radiothread
from ukit.wxui import report
from ukit.wxui import settingsedit
...
class ChirpWelcomePanel(wx.Panel):
    ...
    def __init__(self, *a, **k):
        ...
        locale_base_path = resources.files('ukit.share')
...
class ChirpMain(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        with resources.as_file(
            resources.files('ukit.share').joinpath('plus-icon.png')
        ) as icon:
    ...
    def set_icon(self):
        if sys.platform == 'win32':
            ...
            icon = 'UKIT.png'
        with resources.as_file(
            resources.files('ukit.share').joinpath(icon)
        ) as path:
    ...
    def open_file(self, filename, exists=True, select=True, rclass=None):
        ...
        with common.expose_logs(logging.WARNING, 'ukit.drivers', label,
                                parent=self):
            if exists:
    ...
    def add_stock_menu(self):
        ...
        dist_stock_confs = sorted(
            [
                ...
                in resources.files('ukit.stock_configs').iterdir()
                if conf.is_file()
            ]
        )
    ...
    def _menu_open_stock_config(self, event):
        ...
        with resources.as_file(
            resources.files('ukit.stock_configs').joinpath(fn)
        ) as path:
    ...
    def _menu_import(self, event):
        ...
        r = d.ShowModal()
        if r == wx.ID_YES:
            with common.expose_logs(logging.WARNING, 'ukit.drivers',
    ...
    def _menu_language(self, event):
        ...
        if initial != choices.index(choice):
            wx.MessageBox(_('UKIT must be restarted for the new selection '
                            'to take effect'),
                          _('Restart Required'))
    ...
    def load_module(self, filename):
        ...
        directory.enable_reregistrations()
        self.SetTitle('UKIT **%s**' % _('Module Loaded'))
        ...
        import sys
        modname = 'ukit.loaded.%s' % os.path.splitext(
    ...
    def _menu_load_module(self, event):
        ...
        r = wx.MessageDialog(self,
                             _('Loading modules can be extremely dangerous, '
                               ...
                               'anywhere other than the main UKIT website '
                               '(UniversaKIT.com). Loading a module from '
    ...
    def _menu_about(self, event):
        ...
        d = wx.Dialog(self, title=_('About UKIT'))
        d.SetSize((500, 400))
        vbox = wx.BoxSizer(wx.VERTICAL)
        d.SetSizer(vbox)

        label = wx.StaticText(d, label='UKIT %s' % CHIRP_VERSION)
...
GNU General Public License for more details."""
    ...
    def _menu_developer(self, menuitem, event):
        ...
        if menuitem.IsChecked():
            msg = _(
                ...
                'by developers of the UKIT project, or under the '
        ...
        wx.MessageBox(_('Developer state is now %s. '
                        'UKIT must be restarted to take effect') % state,
    ...
    def _menu_reporting(self, menuitem, event):
        if not menuitem.IsChecked():
            r = wx.MessageBox(
                _('Reporting helps the UKIT project know which '
...
def display_update_notice(version):
    ...
    url = 'UniversalKIT.com'
    msg = _('A new UKIT version is available. Please visit the '
            'website as soon as possible to download it!')
    d = wx.MessageDialog(None, msg, _('New version available'),
                         style=wx.OK | wx.CANCEL | wx.ICON_INFORMATION)
    visit = d.ShowModal()
    if visit == wx.ID_OK:
        wx.MessageBox(_('Please be sure to quit UKIT before installing '
                        'the new version!'))
        webbrowser.open(url)
